...
FUTURE_DATA = pathlib.Path("data/future")  # df à T+1, T+2, T+3

def predict(df: pd.DataFrame | None = None):
    if df is None:
        df = pd.read_parquet(FUTURE_DATA / "projection_an_1.parquet")

    model = joblib.load(MODEL)
    proba = model.predict_proba(df)[:, 1]        # proba bloc A
    return pd.DataFrame({"code_commune": df["code_commune"], "proba_A": proba})
